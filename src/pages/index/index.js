    ...
    chooseImage: function (e) {
        ...
        wx.chooseImage({
            ...
            success: function (res) {
                wx.getImageInfo({
                    src: __WECHAT__ ? res.tempFilePaths[0] : res.apFilePaths[0],
                    success: function (res) {
                ...
                that.setData({
                    file: __WECHAT__ ? res.tempFilePaths[0] : res.apFilePaths[0]
                });

                that.upload(__WECHAT__ ? res.tempFilePaths[0] : res.apFilePaths[0]);
            }
        })
    },
    upload: function (file) {
        ...
        if (file) {
            ...
            const uploadTask = wx.uploadFile({
                url: 'https://starlog.cn/lookup', //仅为示例，非真实的接口地址
                filePath: file,
                name: 'image',
                formData: {
                    'user': 'test'
    ...
    share() {
        ...
        wx.getImageInfo({
            ...
            success: function (res) {
                ...
                ctx.setFontSize(0.037 * windowWidth);
                ctx.setTextAlign('center');
                ctx.fillText('https://starlog.cn/', windowWidth / 2, 0.25  * windowWidth);
